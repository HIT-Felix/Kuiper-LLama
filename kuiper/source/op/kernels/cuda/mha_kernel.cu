...
__global__ void multi_head_attention_kernel(int32_t pos, int32_t seq_len, float* query,
  ...
  const int32_t paged_layer_offset = layer_index * layer_stride;
  ...
  for (int t = threadIdx.x; t <= pos; t += blockDim.x) {
    ...
    if (use_paged_kv) {
      ...
      key_head = key_cache + paged_layer_offset + physical_block_idx * block_stride +
  ...
  for (int i = threadIdx.x; i < head_size; i += blockDim.x) {
    float value = 0.0f;
    if (use_paged_kv) {
      for (int32_t logical_block_idx = 0; logical_block_idx <= pos / block_size; ++logical_block_idx) {
        const int32_t physical_block_idx = block_table[logical_block_idx];
        float* value_block = value_cache + paged_layer_offset + physical_block_idx * block_stride +
                             head_offset;
        const int32_t block_token_begin = logical_block_idx * block_size;
        const int32_t block_token_end = min(pos + 1, block_token_begin + block_size);
        for (int32_t t = block_token_begin; t < block_token_end; ++t) {
          const int32_t token_offset = t - block_token_begin;
          value += score_head[t] * value_block[token_offset * kv_dim + i];
        }
      }
    } else {
      for (int t = 0; t <= pos; ++t) {
